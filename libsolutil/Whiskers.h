...
private:
	...
	/// @param _prefix a vector of strings, where each element is used to compose the tag
	///        like `"<" + element + _parameter + ">"`. Each element of _tags is used as a prefix of the tag name.
	void checkTemplateContainsTags(std::string const& _parameter, std::vector<std::string> const& _prefix) const;
